...
class DesuraReport:
    ...
    def table_local_games(self):
        ...
        tr_head = \
            "<tr>" \
            "<th>#</th><th>Icon</th><th>Name</th><th>Short Name</th><th>EXE</th><th>Play</th><th>Desura Page</th>" \
            "</tr>"
        for listindex, game in enumerate(self.localgames):
            ...
            play = "<td><a class='btn btn-success' href='desura://launch/games/{0}/'>Play</a></td>".format(
                game.shortname
            )
            desura = "<td><a class='btn btn-info' href='http://www.desura.com/games/{0}/'>Desura Page</a></td>".format(
                game.shortname
            )
            tr.append(''.join(['<tr>',index, icon, name, shortname, exe, play, desura, '</tr>']))

        return ''.join(
            ["<table class='table table-striped table-hover table-condensed'>", tr_head, ''.join(tr), '<table>']
        ), len(tr)
    ...
    def table_owned_games(self):
        ...
        for listindex, game in enumerate(self.ownedgames):
            ...
            install = "<td><a class='btn btn-primary' href='desura://install/games/{0}/'>Install</a></td>".format(
                game.shortname
            )
            desura = "<td><a class='btn btn-info' href='http://www.desura.com/games/{0}/'>Desura Page</a></td>".format(
                game.shortname
    ...
    def generate_html(self):
        ...
        desurareport.write(
            "<b>Account: </b><a href='http://www.desura.com/members/{0}'>{0}</a>".format(self.desuraname)
        )
